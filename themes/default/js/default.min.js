...
jQuery(document).ready(function(w){
    //Under Dev
    w('a').click(function(){return false;});
    var image = document.getElementById('bgb');
    image.onload = function() {
        var engine = new RainyDay('canvas','bgb', window.innerWidth, window.innerHeight);
        engine.gravity = engine.GRAVITY_NON_LINEAR;
        engine.trail = engine.TRAIL_DROPS;
        engine.VARIABLE_GRAVITY_ANGLE = Math.PI / 8;
        engine.rain([ engine.preset(0, 2, 0.5), engine.preset(4, 4, 1) ], 50);
    };
    w('article p a,.widget a').each(function(i,d){w(d).html('<span>'+w(d).html()+'</span>');});
    w(window).resize(function(){
        w('#canvas').attr('height',window.innerHeight);
        w('#canvas').attr('width',window.innerWidth);
        w('#canvas').css({'width':window.innerWidth+'px','height': window.innerHeight+'px'});
    });
...
function func_scoll(){
    ...
    if(tmp > jQuery('.navigation').offset().top){
        var tmp_index = parseInt(jQuery('.navigation').data('index'));
        var tmp_total = parseInt(jQuery('.navigation').data('total'));
        if(tmp_total==0 || tmp_index <= tmp_total){
            jQuery.ajax({
                url:lang_url+'page/' + jQuery('.navigation').data('index'),
                type:'get',
                dataType:'json',
                beforeSend:function(){
                ...
                success:function(j){
                    if(j.status){
                        jQuery('.navigation').data('total', j.data.page_count);
                        jQuery('.navigation').data('index', parseInt(j.data.page_now)+1);
                        for(var index in j.data.result){
                            create_html(j.data.result[index],true);
                        }
                    }
                    jQuery(window).bind('scroll',function(){func_scoll();});
                }
            });
        }
    }
...
function create_html(p,expert){
    var html = '<li><div class="meta">\
        <time datetime="'+ p.created_at+'">'+
         (new Date(p.created_at)).Format("yyyy-MM-dd hh:mm:ss.S")+'\
        </time><address>'+ p.terms.city +'</address></div>\
        <div class="device '+ p.device +'"></div>\
        <div class="mrright"><article>\
        <cite>'+ p.address +'</cite>\
        <h2><a href="'+lang_url+ p.slug+'">'+ p.name +'</a></h2><section>';
    if(expert)
        html += p.expert;
    else
        html += p.content;
    html += '</section><section class="meta">'+ p.meta + '</section></article></div></li>';
    jQuery('.timeline').append(html);
}
